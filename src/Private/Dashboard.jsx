...
const Dashboard = () => {
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] text-slate-900 pt-20 md:pt-28 pb-12 px-4 md:px-8 font-sans">
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-3 space-y-6 order-2 lg:order-1">
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden flex flex-col lg:max-h-[75vh]">
             ...
             <div className="p-6 flex flex-col flex-1 overflow-hidden">
                ...
                <div className="space-y-4 overflow-y-auto max-h-60 lg:max-h-none pr-2 custom-scrollbar flex-1">
                  {myOrders.length > 0 ? (
                    myOrders.map(order => (
                      <div key={order._id} className="p-4 bg-slate-50 border border-slate-100 rounded-2xl flex flex-col gap-3 group hover:border-indigo-200 transition-all shadow-sm">
                          <div className="flex justify-between items-start">
                            <div className="flex flex-col">
                              <span className="text-[10px] font-mono font-bold text-slate-500">ID_{order._id.slice(-6).toUpperCase()}</span>
                              <span className="text-[8px] text-slate-400 font-bold uppercase">{new Date(order.createdAt).toLocaleDateString()}</span>
                            </div>
                            <span className={`text-[8px] font-black px-2 py-1 rounded-lg uppercase border transition-colors ${
                          ...
                          <div className="flex flex-wrap gap-2 pt-2 border-t border-slate-200/50">
                            {order.orderItems?.map((item, idx) => (
                              <div key={idx} className="flex items-center gap-2 bg-white p-1.5 rounded-xl border border-slate-100 pr-3">
                                <img 
                                  src={getImageUrl(item.image)} 
                                  alt={item.name} 
                                  className="w-6 h-6 object-contain rounded-md bg-slate-50"
                                />
                                <div className="flex flex-col">
                          ...
                          <div className="flex justify-between items-center mt-1">
                            ...
                            <span className="text-xs font-black text-slate-900 font-mono">₹{Number(order.totalPrice || order.itemsPrice).toFixed(2)}</span>
                          </div>
                      </div>
                    ))
                  ) : (
        ...
        <div className="lg:col-span-9 order-1 lg:order-2">
          <div className="flex flex-col md:flex-row md:justify-between md:items-end mb-8 border-b border-slate-200 pb-6 gap-4">
            <div>
                <h1 className="text-3xl md:text-4xl font-black text-slate-900 uppercase tracking-tighter">Marketplace</h1>
                <p className="text-[10px] md:text-xs text-slate-500 font-bold uppercase tracking-widest mt-1">Inventory Access Terminal</p>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
            {products.map((product) => (
              <div key={product._id} className="bg-white rounded-2xl border border-slate-200 hover:border-indigo-500 transition-all group overflow-hidden shadow-sm hover:shadow-xl">
                <div className="h-48 md:h-52 bg-slate-50 relative flex items-center justify-center p-8">
                ...
                <div className="p-5 md:p-6">
                    <h3 className="font-bold text-slate-800 mb-1 text-base md:text-lg">{product.name}</h3>
                    <div className="flex justify-between items-center mt-4 md:mt-6">
                        <p className="text-xl md:text-2xl font-black text-slate-900 font-mono tracking-tighter">₹{product.price}</p>
                        <button 
      ...
      {showCart && (
        <div className="fixed inset-0 z-[700] flex justify-end">
          ...
          <div className="relative w-full sm:max-w-md bg-white h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
                ...
                {cartItems.length === 0 ? (
                  ...
                  cartItems.map((item) => (
                    <div key={item._id} className="flex items-center gap-4 md:gap-5">
                        <div className="w-14 h-14 md:w-16 md:h-16 bg-slate-50 rounded-2xl border border-slate-100 p-2 shrink-0">
                            <img src={getImageUrl(item.image)} className="w-full h-full object-contain" alt={item.name} />
                        </div>
                        <div className="flex-1 min-w-0">
                            <h4 className="text-[10px] md:text-[11px] font-black text-slate-800 uppercase truncate">{item.name}</h4>
            ...
            <div className="p-6 md:p-8 bg-slate-50 border-t border-slate-200 space-y-4 md:space-y-6 shrink-0">
                ...
                <div className="grid grid-cols-2 gap-3">
                  {[
                    ...
                    <button
                      ...
                      className={`flex flex-col items-center gap-2 p-3 md:p-4 rounded-2xl border-2 transition-all ${paymentMethod === method.id ? 'border-indigo-600 bg-white text-indigo-600 shadow-md' : 'border-transparent bg-slate-200/50 text-slate-400'}`}
                    >
                      {method.icon}
                ...
                {paymentMethod === 'UPI' && (
                  <div className="bg-indigo-50 border border-indigo-100 rounded-2xl p-4 space-y-3 animate-in fade-in slide-in-from-top-2">
                    ...
                    <div className="space-y-2">
                      {upiIds.map((id) => (
                        <div key={id} className="bg-white border border-indigo-200 p-2 rounded-xl flex justify-between items-center px-4 overflow-hidden">
                          <code className="text-[9px] md:text-[10px] font-mono font-bold text-slate-700 truncate mr-2">{id}</code>
              ...
              <div className="flex justify-between items-end pb-2">
                ...
                <span className="text-3xl md:text-4xl font-black text-slate-900 font-mono tracking-tighter">₹{totalPrice}</span>
              </div>
      ...
      {showSuccessModal && (
        <div className="fixed inset-0 z-[800] flex items-center justify-center p-4 md:p-6 backdrop-blur-md">
          ...
          <div className="relative bg-white w-full max-w-sm p-8 md:p-12 rounded-[2.5rem] shadow-2xl text-center transform animate-in zoom-in duration-300">
            ...
            <h2 className="text-2xl md:text-3xl font-black text-slate-900 mb-3 uppercase tracking-tighter">Success</h2>
